...
if [ $1 == "install" ]; then
	...
	if [ ! -e "$HOME/brl-nvidia/nvidia-${driverVersion}.run" ]; then curl https://us.download.nvidia.com/XFree86/Linux-x86_64/${driverVersion}/NVIDIA-Linux-x86_64-${driverVersion}.run -o ~/brl-nvidia/nvidia-${driverVersion}.run; fi
	if [ $2 != "all" ]; then
